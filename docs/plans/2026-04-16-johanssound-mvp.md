# Johanssound MVP Implementation Plan
...
**Acceptance:** `Johanssound.xcodeproj` opens in Xcode, has macOS and iOS app targets, builds and runs a "Hello Johanssound" window on both. Shared SwiftUI code in a framework target.

### Task 0.1: Create Xcode project structure

**Files:**
- Create: `Johanssound.xcodeproj` (via Xcode UI or `xcodegen`)
- Create: `JohanssoundMac/JohanssoundMacApp.swift`
- Create: `JohanssoundIOS/JohanssoundIOSApp.swift`
- Create: `JohanssoundCore/JohanssoundCore.swift` (shared framework)
- Create: `JohanssoundCore/Views/HelloView.swift`
...
name: Johanssound
options:
  bundleIdPrefix: se.jonasjohansson.johanssound
  deploymentTarget:
    macOS: "14.0"
    iOS: "17.0"
targets:
  JohanssoundCore:
    type: framework
    platform: [macOS, iOS]
    sources: JohanssoundCore
  JohanssoundMac:
    type: application
    platform: macOS
    sources: JohanssoundMac
    dependencies:
      - target: JohanssoundCore_macOS
  JohanssoundIOS:
    type: application
    platform: iOS
    sources: JohanssoundIOS
    dependencies:
      - target: JohanssoundCore_iOS
```

**Step 3: Create `JohanssoundCore/Views/HelloView.swift`**
...
**Step 4: Create `JohanssoundMac/JohanssoundMacApp.swift`**
...
**Step 5: Create `JohanssoundIOS/JohanssoundIOSApp.swift`**
...
```bash
cd /Users/jonas/Documents/GitHub/org/jonasjohansson/johanssound
xcodegen generate
xcodebuild -scheme JohanssoundMac -destination 'platform=macOS' build
xcodebuild -scheme JohanssoundIOS -destination 'generic/platform=iOS Simulator' build
```
...
**Files:**
- Create: `JohanssoundCore/Tests/HelloViewTests.swift`

**Step 1: Add a test target to `project.yml`**

```yaml
  JohanssoundCoreTests:
    type: bundle.unit-test
    platform: macOS
    sources: JohanssoundCore/Tests
    dependencies:
      - target: JohanssoundCore_macOS
```
...
```bash
xcodegen generate
xcodebuild test -scheme JohanssoundCore -destination 'platform=macOS'
```
...
**Files:**
- Create: `JohanssoundCore/Library/LibraryConfig.swift`
- Create: `JohanssoundCore/Views/FolderPickerView.swift`
- Create: `JohanssoundCore/Tests/LibraryConfigTests.swift`

**Step 1: Write failing test for `LibraryConfig` persistence**

```swift
func testLibraryConfigPersistsFolderURL() throws {
    let tempURL = URL(fileURLWithPath: "/tmp/johanssound-test")
...
```bash
xcodebuild test -scheme JohanssoundCore -destination 'platform=macOS' -only-testing:JohanssoundCoreTests/LibraryConfigTests/testLibraryConfigPersistsFolderURL
```
...
public final class LibraryConfig {
    ...
    private let key = "johanssound.musicFolder"
...
**Files:**
- Create: `JohanssoundCore/Library/Track.swift`
- Create: `JohanssoundCore/Library/LibraryIndexer.swift`
- Create: `JohanssoundCore/Tests/LibraryIndexerTests.swift`
- Test fixtures: `JohanssoundCore/Tests/Fixtures/library/`

**Step 1: Create test fixtures**

```bash
mkdir -p JohanssoundCore/Tests/Fixtures/library/ArtistA/AlbumA
...
**Files:**
- Create: `JohanssoundCore/Views/LibraryView.swift`
- Create: `JohanssoundCore/ViewModels/LibraryViewModel.swift`
...
**Step 3: Wire up in `JohanssoundMacApp`**
...
**Files:**
- Create: `JohanssoundCore/Player/AudioPlayer.swift`
- Create: `JohanssoundCore/Tests/AudioPlayerTests.swift`
...
**Files:**
- Create: `JohanssoundCore/Views/PlayerView.swift`
...
**Files:**
- Create: `JohanssoundCore/Player/NowPlayingController.swift`
...
- Design doc: `../../skynet/wiki/projects/johanssound.md`
