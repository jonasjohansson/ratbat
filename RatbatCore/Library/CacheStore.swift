...
/// as a hidden dotfile (`.ratbat-cache.json`) so a subsequent launch can
...
enum CacheStore {
    ...
    static let cacheFilename = ".ratbat-cache.json"
    ...
    /// Read and decode the cache file sitting at `{root}/.ratbat-cache.json`.
